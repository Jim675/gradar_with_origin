...
int loadBasedataImage(const char* fpath, struct basedataImage* gdi)
{
    ...
    while (1) {
        ...
        size_t ret = fread(pgrad, 1, GENE_RAD_HEADER_LEN, fp);
        ...
        for (int i = 0; i < grad.momNum; i++) {
            ...
            */
            uniCopyToShort(&gm.points[0], &gRadBuffer[pos], binnum, binsize);
...
int writeBasedataImage(const char* fpath, struct basedataImage* gdi)
{
    ...
    for (it = gdi->radials.begin(); it != gdi->radials.end(); it++) {
        ...
        size_t ret = fwrite(pgrad, 1, GENE_RAD_HEADER_LEN, fp);
        if (ret != GENE_RAD_HEADER_LEN) {
        ...
        for (iit = it->mom.begin(); iit != it->mom.end(); iit++) {
            ...
            int binnum = len / binsize;
            uniCopyFromShort(gRadBuffer, &iit->points[0], binnum, binsize);
...
void decodeMomData(const cv_geneMom& gm, float* points, float def)
{
    for (int i = 0; i < gm.points.size(); i++) {
        if (!isSpecCode(gm.points[i]))
            points[i] = decodeData(gm.udt.offset, gm.udt.scale, gm.points[i]);
        else
            points[i] = def;
    }
...
void updateRadailLength(struct basedataImage& bdi)
{
    ...
    for (it = bdi.radials.begin(); it != bdi.radials.end(); it++) {
        ...
        size_t radLen = 0, momNum = 0;
        for (iit = it->mom.begin(); iit != it->mom.end(); iit++) {
            size_t momLen = iit->udt.binSize * iit->points.size();
